...
This script was particular made to run simulations using CASCADE 1.0 and CASCADE 2.0 topologies (and the respective input files), while changing some of gitsbe's and drabme's configuration parameters (e.g. the number of simulations and the synergy calculation method).

In particular, executing the script from the root of the `druglogics-synergy` repository with no changes:

```
./run_druglogics_synergy.sh
```

will run the synergy prediction pipeline (gitsbe and drabme) a total of $16$ times, that is $2$ topologies (CASCADE 1.0 and 2.0) $\times 2$ (training to the AGS steady state and a random proliferative data profile) $\times 2$ ($50$ and $150$ simulations in the genetic algorithm) $\times 2$ (HSA and Bliss as synergy method).
Also all these simulations will compute the fixpoint attractors.

Following the above logic, the given script can be extended to run multiple simulations based on the input directories and given configuration options.
